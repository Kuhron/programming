...
def select_survivors(dnas, eval_func, birth_factor, death_factor):
    ...
    print(f"killed {n_killed} of {len(dnas)} individuals; cutoff fitness {kill_cutoff_val}, max fitness {max_fitness}")
    return survivors
...
def run_evolution_in_slightly_different_environments(eval_func, n_params):
    ...
    environment_deviations = [np.random.normal(0, 0.01, n_params) for i in range(n_environments)]
...
if __name__ == "__main__":
    def eval_func(dna, params):
        ...
        return np.tanh(len(dna)) - abs(top_bottom_ratio_bidirectional - desired_top_bottom_ratio)

    run_evolution_in_slightly_different_environments(eval_func, n_params=5)
